...
int main(int argc, char const *argv[]) {
    int sockfd;
    struct sockaddr_in servaddr, tldaddr, local_addr;
    char request[100];
    ...
    memset(&tldaddr, 0, sizeof(tldaddr));
    tldaddr.sin_family = AF_INET;
    tldaddr.sin_port = htons(DNS_PORT);
    tldaddr.sin_addr.s_addr = inet_addr("127.0.0.22");
    ...
    while (1) {
        ...
        if(flag==0){
             int lenn = sizeof(tldaddr);
       	     if(sendto(sockfd, request, strlen(request), 0, (struct sockaddr *) &tldaddr, lenn) < 0) {
            	perror("Could not send data\n");
            	exit(1);
        	}
             int nn = recvfrom(sockfd, response, 100, 0, (struct sockaddr *) &tldaddr, &lenn);
             response[nn]='\0';
